...
import { runAssistantChat } from './assistantHandler.js';

function createAssistantMiddleware(): Connect.NextHandleFunction {
  return (req: IncomingMessage, res: ServerResponse, next: Connect.NextFunction) => {
    const url = req.url?.split('?')[0] ?? '';
    if (url !== '/api/assistant' && url !== '/api/assistant/') {
      next();
      return;
    }
...
export function galaxyAssistantApiPlugin(env: Record<string, string>): Plugin {
  Object.assign(process.env, env);

  const attach = (server: { middlewares: Connect.Server }) => {
    server.middlewares.use(createAssistantMiddleware());
  };
